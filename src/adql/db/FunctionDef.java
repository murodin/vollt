 ...
 * 	as the one used to build the string returned by {@link #toString()}. Here is
 * 	this syntax:
 * </p>
...
public class FunctionDef implements Comparable<FunctionDef> {
	 ...
	 * {@link #regularIdentifierRegExp}) and a type (see {@link #typeRegExp}). */
	 ...
	 * 		each kind of type (in the order): numeric, string, geometry.
	 * 	    Each character is either a 0 or 1, so that indicating whether the
	 ...
	 * 	{@link #isString()} and {@link #isGeometry()}
	 * 	<b>MUST ALL</b> return <code>false</code>. Otherwise, one of these 3
	 * 	last functions MUST return <code>true</code>.
	 * </p>
	 *
	 * @return	<code>true</code> if this function returns an
	 *        	unknown/unresolved/unsupported type,
	 *        	<code>false</code> otherwise.
	 */
	 ...
	 *                	parameter or any other parameter. These latter should
	 *                	have been separated from the datatype before calling
	 *                	this function. It can however contain space(s) in first,
	 *                	last or intern position.</i>
	 ...
	 * <ol>
	 * 	<li><b>Name test:</b>
	 * 		if the name of both function are equals, next steps are evaluated,
	 * 		otherwise the standard string comparison (case insensitive) result
	 * 		is returned.</li>
	 * 	<li><b>Parameters test:</b>
	 * 		parameters are compared individually. Each time parameters (at the
	 * 		same position in both functions) are equals the next parameter can
	 * 		be tested, and so on until two parameters are different or the end
	 * 		of the parameters' list is reached. Just the kind of type is used
	 * 		for parameter comparison. Each kind of type is tested in the
	 * 		following order: numeric, string and geometry. When a kind of type
	 * 		is not equal for both parameters, the function exits with the
	 * 		appropriate value (1 if the parameter of this function definition is
	 * 		of the kind of type, -1 otherwise).</li>
	 * 	<li><b>Number of parameters test:</b>
	 * 		in the case where this function definition has N parameters and the
	 * 		given ADQL function has M parameters, and that the L (= min(N,M))
	 * 		first parameters have the same type in both functions, the value
	 * 		returns by this function will be N-M. Thus, if this function
	 * 		definition has more parameters than the given function, a positive
	 * 		value will be returned. Otherwise a negative value will be returned,
	 * 		or 0 if the number of parameters is the same.</li>
	 * </ol>
	 *
	 * <p><i><b>Note:</b>
